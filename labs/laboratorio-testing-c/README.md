...
> R: solo muestra el precio de un item

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R: multiplicar por la cantidad de items
...
> R: el <= es incorrecto, deberia ser un < excluyente
...
> R: Por que no hay fallas y es una simple cuenta, incluso el divisor ya esta predefinido y no es 0 
...
> R: el unitario prueba una sola función aislada, el de integración prueba que varias funciones se comuniquen bien entre sí. El unitario lo detectó primero.
...
> R: Porque C no controla los límites de los arreglos al ejecutarse, dejando sobrescribir memoria crítica. Python o Java sí los controlan y lanzan un error antes de corromper nada.
...
> R: Al leer el código es fácil pasar errores lógicos por alto. Los tests dan pruebas reales, evalúan casos límite y quedan guardados para automatizar revisiones a futuro.
...
> R: Porque el error era que faltaba multiplicar por la cantidad. Como en el primer test la cantidad era 1, el resultado era el mismo matemáticamente (precio == precio * 1), ocultando el bug.
